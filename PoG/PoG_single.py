...
# extracting possible entities from the response generated by an LLM 
def extract_possible_entities(text):
    print ("planing extract_possible_entities")

    # Define a regex pattern to capture entities inside brackets or quotes, and find all matches
    # The pattern searches for:
        # Entities inside double quotes → "(.*?)"
        # Entities inside curly brackets {} → {(.*?)}
    # eg. "Denmark", "Germany", {m.0f8l9c, m.03d1}
    pattern = r'edicted:\s*(?:"(.*?)"|{(.*?)})'
    matches = re.findall(pattern, text)
    
    results = []
    for match in matches:
        # Combine the extracted parts and remove whitespace
        combined = ' '.join([m.strip() for m in match if m])

        # Split the extracted entity list by commas and strip whitespace
...
# extract unqiue path head/tails
...
# quickly eliminate irrelevant reasoning paths using a pre-trained embedding model (SentenceBERT)
# 1. Encode all candidate paths into vector embeddings (HPaths).
# 2. Encode an LLM-generated indicator (ILLM) into vector embeddings (HI)
# 3. Compute cosine similarities between HPaths and HI.
# 4. Select the top W1 paths with the highest similarity scores for further processing.
def Beam_search_step1(query_sentence, NL_CoT_all_paths, total_id_to_name_dict, top_k_value = 80):
    print('''
        ...
        ''')
    if not NL_CoT_all_paths:    # if empty
        return []
    ...
    if len(NL_path_tobe_del) >= top_k_value:
        # Uses a pre-trained SentenceBERT model to compute embeddings.
        # 加载模型
...
# Eliminate redundant or unnecessary branches from the reasoning paths
# achieved by iteratively extracting partial paths (initial steps) and pruning excessive branches using a graph structure-aware method
def Beam_search_step2(NL_CoT_all_paths, version, total_id_to_name_dict, query_sentence, top_k_value = 40):
    print('''
        ...
        ''')
    # extract initial 3-hop segments from each path.
    intial_hop = 0
    ...
    while len(NL_CoT_all_paths) > top_k_value:
        ...
        # if number of unqiue heads < 40, increase depth to extract more heads
        ...
        # Apply LLM-Based Path Selection
        # while(len(NL_tobe_del) > 3):
        NL_tobe_del = head_3
        ...
        # Filter out paths that do not match the selected prefix
        print("number of NL_CoT_all_paths, before remove:", len(NL_CoT_all_paths))
...
def Beam_search_step3(NL_CoT_all_paths, version, total_id_to_name_dict, query_sentence, top_k_value = 3):
    ...
    while len(NL_CoT_all_paths) > top_k_value:
        # prunned_path_number = beam_path_select(question, split_answer,data, NL_CoT_all_paths)
        token_length = num_tokens_from_string("".join(NL_CoT_all_paths), "cl100k_base")     # Ensures LLM input does not exceed token constraints.
        input_token_length(token_length)
...
# Refines paths based on ranking criteria, ensuring only the most meaningful paths are kept.
# question: The original question being processed.
# split_answer: The LLM-generated reasoning breakdown of the question.
# query_sentence: The CoT reasoning sentence.
# data: The dataset entry for the question (in the dataset).
# total_id_to_name_dict: Maps entity IDs to human-readable names.
# CoT_all_paths: The list of input paths before pruning.
# version: beam width
# eg. Beam_search(
#     "What country borders France and contains an airport that serves Nijmegen?",
#     ["border", "airport", "Nijmegen"],
#     "Nijmegen - service by - airport - owned by - answer(country) - border with - France",
#     dataset_entry,
...
def Beam_search(question, split_answer,query_sentence, data, total_id_to_name_dict, CoT_all_paths, final_entity_path = [], version = 1):
    ...
    if not CoT_all_paths:   # if no paths
        return [],[]
    
    # convert into natural language if haven't done so
    if not isinstance(CoT_all_paths[0], str): 
        NL_CoT_all_paths = format_paths_to_natural_language_id_with_name(CoT_all_paths,total_id_to_name_dict)
    else:
        NL_CoT_all_paths = CoT_all_paths
    ...
    # only Fuzzy Selection
    if using_beam_step1_only:
        print("test with only step1 - fuzzy reduction")
        initial_path = Beam_search_step1(query_sentence, NL_CoT_all_paths, total_id_to_name_dict, 80)       # Uses a high threshold (80 paths) initially
        NL_CoT_all_paths = Beam_search_step1(query_sentence, NL_CoT_all_paths, total_id_to_name_dict, 3)    # then narrows to 3 paths
        return NL_CoT_all_paths,initial_path
    else:
        NL_CoT_all_paths = Beam_search_step1(query_sentence, NL_CoT_all_paths, total_id_to_name_dict, 80)   # 80 paths
    ...
    NL_CoT_all_paths = Beam_search_step2(NL_CoT_all_paths, version, total_id_to_name_dict,query_sentence, 20)   # Branch Reduction
    NL_CoT_all_paths = Beam_search_step3(NL_CoT_all_paths, version, total_id_to_name_dict,query_sentence, 3)    # Precise Selection
...
# Uses a Chain-of-Thought (CoT) approach to extract missing entities
def CoT_entity_expand_inchain(question, split_answer, data, main_path = None):
    ...
    # If previously discovered reasoning paths (main_path) exist, they are added to the prompt.
    if main_path:
        main_path_prompt = ""
    ...
    try:
        ...
        GPT_entity = extract_possible_entities(question_answer)     # extract entitites that LLM thinks are important
        CoT_list = extract_cots_as_strings(question_answer)         # extract CoT
        if len(CoT_list) == 0:
            print("LLM reasonning error, requsting again")
            print(question_answer)
            time.sleep(120)
...
# Asks the LLM to generate a concise reasoning summary that correctly follows KG triples
# related_edge Extracted paths from the knowledge graph.
def check_n_explor_v4(question, data,  split_answer, related_edge, CoT, prompt_formate):

    # prompt + question + topic entities
    prompt_Cot = prompt_formate + "\nQ:\n Question: " + question+ "?\n"
    prompt_Cot += "Main Topic Entities: \n" + str(data['topic_entity']) + "\n"
...
# Takes a text string as input, which is expected to contain a reasoning chain (Thinking CoT) generated by the LLM.
# Return: 
#   path_length: the estimated number of hops in the reasoning chain
#   thinking_cot_line: extract the CoT statement (string)
def extract_path_length_from_text(text):
    back_up = text
    tokens = re.split(r'\s*-\s*', text.strip())     # Splits text based on " - " separators (which represent relationships in the reasoning chain).

    # 计算路径长度
    # Each relationship is represented as "Entity - Relation - Entity".
    # The total number of tokens should be 1 + 2 * path_length (entities and relations alternate).
    path_length = (len(tokens) - 1) // 2

    # extract the line that contains "cot:"
...
# Identifying lines that contain the word "split" in the LLM response.
...
def extract_split_questions(text):
    ...
    for line in lines:
        ...
        if re.search(r'split', line_no_spaces, re.IGNORECASE):
            # 使用 ':' 分割，提取问题部分
            parts = line.split(':', 1)  # Extracting the portion of the line after the : separator.
            if len(parts) > 1:
...
# Recomputes entity relevance despite the fact that the dataset already provides entity IDs for the question
# - This step ensures that entities remain consistent even if the LLM paraphrases terms.
# - To Handle Entity Ambiguity in the QA Dataset
# - To Extend the Set of Entities for Path Exploration
def find_top_similar_entities(
    ...
    - query_sentence (str): The CoT sentence.
    - top_k (int): The number of top similar entities to retrieve (default is 3).
    - topic_existing: List of entities already selected, prevents duplicates.
    - sbert_model (SentenceTransformer, optional): Pre-trained SentenceTransformer model for embeddings (not used in active code).
    - ner_pipeline (transformers.pipeline, optional): Named Entity Recognition (NER) model for entity extraction (not used in active code).
    - device (str or int): Defines where models should run ('cuda', 'cpu', or a specific GPU index).
    ...
    # Prepare data
    entity_names = list(entity_id_to_name.values())     # eg. entity_names = ["Germany", "France", "Europe"]
    entity_ids = list(entity_id_to_name.keys())         # eg. entity_ids = ["m.02jx3", "m.0f8l9c", "m.0g7qf"]
    ...
    # Splits query_sentence into meaningful segments using \s-\s as a delimiter.
    # eg. input = "Nijmegen - service by - airport - owned by - answer(country) - border with - France"
    #     output = ["Nijmegen", "airport", "answer(country)", "border with", "France"]
    parts = re.split(r'\s-\s(?![^()]*\))', query_sentence)
    
    # 清理每个部分，移除引号和前后空白
    topic_entities = [re.sub(r'[“”"]', '', part).strip() for part in parts]     # Clean each extracted part by removing quotes and whitespaces

    # # Remove already existing topic entities
    for i in topic_entities:
    ...
    # Combine known entities and extracted topic entities
    # eg. corpus = ["Germany", "France", "Europe", "Nijmegen", "airport", "answer(country)", "border with", "France"]
    ...
    # Split TF-IDF matrix into entity vectors and topic entity vectors
    entity_tfidf = tfidf_matrix[:len(entity_names)]     # for existing entities 
    topic_tfidf = tfidf_matrix[len(entity_names):]      # for newly identified topics

    # Compute mean topic entity vector
    topic_vector = topic_tfidf.mean(axis=0)

    # Convert to numpy array    
    topic_vector = topic_vector.A1  # 或者使用 .toarray().ravel()

    # Compute cosine similarity between entity vectors and average topic entity vector
    similarities = cosine_similarity(entity_tfidf, topic_vector.reshape(1, -1))

    # 将相似度数组展平
    # eg similarities = [0.82, 0.90, 0.76]  # Corresponds to ["Germany", "France", "Europe"]
    similarities = similarities.flatten()

    # Select the top_k Most Similar Entities
    top_k_indices = similarities.argsort()[-top_k:][::-1]   # Finds the indices of the top_k highest similarity scores.

    # eg. top_k_indices = [1, 0, 2]  # Corresponds to ["France", "Germany", "Europe"]
    ...
    # top_entities = [
    #     ("m.0f8l9c", "France", 0.90),
    #     ("m.02jx3", "Germany", 0.82),
    # ]
    # topic_entities = ["Nijmegen", "airport", "answer(country)", "border with"]
...
if __name__ == '__main__':
    ...
    # expects 6 command-line arguments after the script name, sys.argv[0] is script name
    if len(sys.argv) < 7:
    ...
    # --------------------------- dataset -------------------------------
    file_name = sys.argv[1]     # dataset name
    # db_path = f'{file_name}_subgraph.db'

    # database files
    subgraph_db = f'subgraph/{file_name}_main_Subgraphs.db'         # stores the subgraph extracted from the KG
    NL_subgraph_db = f'subgraph/{file_name}_main_nl_Subgraphs.db'   # stores a natrual language version of the subgraph
    path_db = f'subgraph/{file_name}_path_db.db'                    # stores paths used for reasoning

    # --------------------------- path summary -------------------------------
    answer_add = ""             # output file name
    if sys.argv[2] == "sum":
        using_summary = True
        answer_add = "_sum"
    else:
    ...
    if sys.argv[3] == "1":              # only use fuzzy selection
        using_beam_step1_only = True
        answer_add += "_BS1"
    elif sys.argv[3] == "12":           # fuzzy selection + branch reduce selection
        using_beam_step1_2 = True
        answer_add += "_BS12"   
    elif sys.argv[3] == "13":           # fuzzy selection + precise selection
        using_beam_step1_3 = True
        answer_add += "_BS13"
    elif sys.argv[3] == "123":          # fuzzy selection + branch reduce selection + precise selection
        answer_add += "_BS123_20"

    # --------------------------- relation usage -------------------------------
    # when retrieving paths from the KG, PoG considers all possible relations for a given entity
    # instead of exploring all relations of an entity, PoGE randomly selects only one relation per entity
    non_allr = answer_add
    if sys.argv[4] == "PoG":
        if_using_all_r = True
        answer_add += "_allr"
    ...
    # --------------------------- language model -------------------------------
    if sys.argv[5] == "gpt4":
        LLM_model = "gpt4"      # default is gpt3, see line 27
        changemode("gpt4")      # does nothing
    ...
    # load dataset
    #   datas: the QA dataset in JSON form
    #   question_string: the key used to extract questions
    #   Q_id: the key used to extract question IDs
    ...
    for LLM_model in [m]:
        ...
        for Global_depth in [Global_depth]:
            ...
            # initialize a SQLite database for storing large data
            ...
            # -------------------------------- Process QA pairs --------------------------------- 
            # Iterates over each of the first 100 QA pairs, tqdm creates progress bar
            ...
            # for data in tqdm(datas[382:400]):
                ...
                # Extract data from each QA
                question = data[question_string]        # the natural language question (str)
                topic_entity = data['topic_entity']     # the topic entities (dict)
                question_id = data[Q_id]                # the unique identifier of this QA (str)

                # Retrieve the real answer (list)
                question_real_answer =  check_answerlist(file_name, question_string, question, datas, data)
                
                if len(topic_entity) > 1:   # skip multi-entity questions
                    continue
                else:
                    inital_num()    # reset global counters

                # Tries to load a previously computed answer from the database
                answer = load_from_large_db(answer_db, question_id)
                # if answer:
                #     print("answer is found in the database")
                #     continue

                # If an answer is found, skip this question unless only use Fuzzy selection
                # the question-answer (QA) reasoning has already been computed in a previous run, 
                ...
                if len(topic_entity) == 1:
                    ...
                    # Builds a prompt for the LLM to split the main question into sub-questions
                    prompt_split = split_question_prompt + "\nQ:\n Question: " + question + "?"+ "\n" # +"\nThe question also writen as: "+ data["machine_question"] 
                    prompt_split += "Main Topic Entities: \n" + str(data['topic_entity']) + "\n" +"A:\n"
                    
                    # prompt the LLM
                    split_answer = run_LLM(prompt_split, LLM_model)[2:]     # string, slice the first 2 characters (Sometimes, LLM responses start with unwanted characters)
                    Num_run_LLM += 1
                    print(f"split_answer: {split_answer}")

                    # extract predicted length, CoT statement, and split-questions
                    ...
                    # Preparing for knowledge graph exploration based on the topic entity extracted from the question.
                    # Initialize: entity tracking, graph data structure, storage for reasoning path
                    print("single entity")
                    print("explore the graph")
                    topic_entity_ids = list(topic_entity)   # extract the keys (IDs) and form a list, graph operation requires ID
                    ...
                    # initialize entity tracking sets
                    ...
                    # Initialize Storage different types of reasoning paths during graph exploration.
                    final_entity_path = []
                    ...
                    print('''
                        ...
                        ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++                  
                        ''')
                    # Expand the KG from the topic entity up to Global_depth
                    ...
                    # explore_graph_from_entities_by_hop_neighbor_1(list(topic_entity), 5, question_real_answer)
                        ...
                        ''')
                        # graph -> update graph after 1-round expansion
                        # all_entities -> includes new entities visited during expansion
                        # exlored_entities -> update entities that have been explored from
                        # next_exlore_entities -> include entities to be explored from next round
                        # total_id_to_name_dict -> updated as entity_names, which contain newly visited entities
                        ...
                        # expand up to predict_length in the first round, then one step each round until Global_depth
                        ...
                        if depth == min(predict_length,3):
                            all_paths = [i for i in all_paths if len(i) > 1]    # all paths
                        else:
                            all_paths = [i for i in all_paths if len(i) == 1+2*depth]   # Filters paths to match the expected length at the current depth.

                        print("number of all_paths:", len(all_paths))

                        # ----------------------------------------  Identifying Additional Relevant Entities ----------------------------------------

                        # Searches for entities that are similar to those already listed in the QA dataset
                        # existing topic entities (i.e., those already in the dataset) are not included in the search for new entities.
                        ...
                        #     ("m.02jx3", "Germany", 0.82),
                        # ]
                        # key_words = ["Nijmegen", "airport", "answer(country)", "border with"]
                        ...
                        if len(key_words) > 0:
                            ...
                            for entity_id, entity_name, similarity_score in top_entities:
                                ...
                                print(f"Entity ID: {entity_id}, Entity Name: {entity_name}, Similarity: {similarity_score:.4f}")
                                if similarity_score < 0.95:     # only keep highly relevant new entities (above a 0.95 similarity threshold)
                                    continue
                        ...
                        print('''
                            ...
                            ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
                            ''')
                        # The purpose of this block is to filter and refine the discovered paths (all_paths) before finalizing them for reasoning.

                        # If more than 3 paths were found, further processing (pruning) is applied.
                        if len(all_paths) > 3:
                            print("number of all_paths:", len(all_paths))
                            pruned_path = final_entity_path + format_paths_to_natural_language_id_with_name(all_paths, total_id_to_name_dict)    # Converts all_paths into a readable format by replacing entity IDs with human-readable names.
                            print("pass1")
                            
                            previouse_all_entity_path = pruned_path     # keep track of the previous set of paths
                            ...
                            # extract path entities from the selected paths
                            ...
                            # Filters next_exlore_entities to include only those present in final_path_entities. 
                            # Ensures that only meaningful entities are expanded in the next search iteration.
                            ...
                            # directly accepts all paths
                            final_entity_path = format_paths_to_natural_language_id_with_name(all_paths,total_id_to_name_dict)  # convert to readable format
                            previouse_all_entity_path = final_entity_path

                        # print the path selections
                        ...
                        if "{Yes}" in result:
                            ...
                            delete_data_by_question_id(answer_db, question_id)      # Remove Any Previous Stored Answer for This Question
                            save_to_large_db(answer_db, question_id, answer_dict)   # Store the New Answer in the Database
                    ...
                    # CoT_entity -> new entities predicted by LLM
                    # CoT_list -> new CoTs predicted by LLM
                    ...
                    # entities that needs to be explored (new entities)
                    ...
                    if len(CoT_high_entity) > 0:
                        CoT_high_entity = reorder_entities(",".join(CoT_entity), CoT_high_entity)   # sort new entities

                        print("exploring CoT predict: ", CoT_high_entity)

                        # expand the Knowledge Graph (KG) using LLM-predicted entity "name" and explore new paths
                        for name in CoT_high_entity:
                            ...
                            # check if "name" exists in previous paths
                            for pre_path in previouse_all_entity_path:
                                if name in pre_path:
                                    check_previouse_path.append(pre_path)

                            # If it exists in previouse_all_entity_path, then we don't need to do additional exploration.
                            if len(check_previouse_path) > 0:
                                ...
                                Exp_list = get_name_to_id(name, total_id_to_name_dict)  # fetches the entity’s ID(s)

                            # searches for the CoT step that contains the entity
                            CoT_indicter = ""
                            for i in CoT_list:
                                if name in i:
                                    CoT_indicter = i
                            ...
                            # Reordering the Entities for Exploration (existing entities + this new entity)
                    ...
                    Total_GPT = []  # stores all the paths found in this exploration step.
                    explored_path_entity = []   # keeps track of already explored entities to avoid redundant searches.
                    current_explored_depth = 1  # Starts with one-hop exploration, then expands iteratively.

                    while current_explored_depth <= Global_depth:
                        # Extracts all entities from previously explored paths
                        candidate_list1 = []
                        candidate_id_list1 = []
                        main_path_entities = extract_entities_from_strings(final_expand_path+Total_GPT)
                        print("main_path_entities:", main_path_entities)
                    
                        # Identify candidate entities that haven't been explored yet:
                        ...
                        # Performing 1-Hop Exploration on New Entities
